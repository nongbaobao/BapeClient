...
public class womanbox implements ICommand {

	private Minecraft mc = Minecraft.getMinecraft();

	public Client vapuClient =null;

	public womanbox(Client vapuClient) {
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		if (args.length == 1 && !ModuleManager.getModule("NoCommand").getState()) {
			try {
				mc.thePlayer.sendChatMessage("姓名："+ esufaker.getName("woman")+" 身份证："+ esufaker.getSFZ(args[0]));
			}
			catch(Exception e) {
			}
		}
